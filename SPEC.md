...
5. The Job submits the Recording to the Provider as two requests: the Remote Track with diarization on and a speaker range; the Mic Track without diarization (its single speaker is structural — the Adapter labels all of it "Me"). Both requests set English (`en_au`) explicitly and attach Key Terms. No connectivity → the Job queues and retries.
6. The Adapter merges both results into one Transcript on the shared Recording clock (merge rule in Architecture).
7. The Summariser (Claude, `claude-opus-5`) produces the Note from the Transcript using the Preset; Participant names are used only where the Transcript makes them evident, and only for Remote speakers — the Mic speaker is always "Me". The Note is written to the Vault, the Transcript to `<Vault>/transcripts/`; the Recording is deleted only after both the Note and the Transcript are confirmed on disk.
...
R6. The Adapter submits the Remote Track with diarization enabled and `speaker_options` range {min 1, max = Participants count + 1, or 6 when no Participants given}, the Mic Track without diarization, and both with `language_code: en_au` and the Key Terms list. | Check: assert against `AssemblyAIAdapter.requestBody(...)` and `remoteSpeakerRange(...)` — the same functions the live path calls. (The pipeline log carries these too, for reading in Console.app; `log show` is unavailable to the build environment, so it cannot be the automated check.)
...
## Design
Native, minimal, no web frontend of any kind. UI surface: menu-bar icon with the five R15 states; Start popover (Preset, Title, Participants); Pause/Resume/Stop controls; settings window (Vault path, API keys, Key Terms, Presets, cost total); macOS notifications for "Note ready" and failures.
...
**State**: Job state and pending Recordings in `~/Library/Application Support/ms-notes/`. Signing: the stable self-signed local identity "ms-notes Development" ([ADR-0004](docs/adr/0004-swiftpm-no-xcode-toolchain.md)), or macOS's privacy-permission system (TCC) re-prompts and misbehaves between builds.
