...
R18. Delivered Sessions are logged so the app can show its own history. The panel lists the recent ones, each opening its Note, and shows this calendar month's minutes against a user-set cap alongside the month's spend. Reaching the cap warns and never blocks recording. | Check: `sessionIndexKeepsNewestFirstAndTrimsTheTail`, `usageCountsThisMonthOnly`, `usageFlagsTheCapWithoutEverBlocking`, `daysLeftCountsTodayAndResetsOnTheFirst`.
