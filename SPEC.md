...
R6. The Adapter submits the Remote Track with diarization enabled and `speaker_options` range {min 1, max = Participants count + 1, or 6 when no Participants given}, the Mic Track without diarization, and both with `language_code: en_au` and the Key Terms list. | Check: inspect the pipeline log (Operation) for a test Session's request parameters.
